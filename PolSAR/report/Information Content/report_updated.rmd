...
## conversão de original para espaço lab
...
#conversão de ideal para espaco Lab usando a referência da luz D65.
...
## visualização de imagem original em RGB e Lab, e sua histograma
par(mfrow = c(1, 3))

## visualização de original em RGB
plot(originalRGB)
title("Visualização da mandrill-RGB")

## visualização de original em Lab
plot(originalLab)
title("Visualização da mandrill-Lab")

## visualização de histograma do componente L de original
hist(L_O, breaks = "FD", xlab = "L",
                                 ylab = "Frequência",
     main = "Componente L de mandrill")


## visualização da imagem ideal em RGB e Lab, e a sua histograma. 

## tansformação dos componentes do ideal
## em objetos cmig para plotagem em RGB
iR <- as.cimg(idealRGB$X1)
iG <- as.cimg(idealRGB$X2)
iB <- as.cimg(idealRGB$X3)

## transformação dos componentes do ideal em
## objetos cmig para plotagem em Lab
iL <- as.cimg(idealLab$L)
ia <- as.cimg(idealLab$a)
ib <- as.cimg(idealLab$b)


par(mfrow = c(1, 3))
## visualização de ideal em RGB
imappend(list(iR, iG, iB), "C") %>% plot
title("Ideal em espço RGB")

## visualização de ideal em Lab
imappend(list(iL, ia, ib), "C") %>% plot
title("Ideal em espço Lab")

## visualização de histograma do componente L de ideal
...
```{r echo=FALSE}
   ...
   ## equalização do componente L de originalLab
   L_eq <- equalize(L_O)

   ## forçando histograma de componente L de originalLab à 
   ...
   ## subtituição do componente L da nova imagem para L_H
   nova_originalLab[,,,1] <- L_H
  
   par(mfrow = c(1, 3))
   ## visualização de nova imagem
   plot(nova_originalLab)
   ...
   hist(L_eq, main = "Histograma L do originalLab equalizada", "FD")
```